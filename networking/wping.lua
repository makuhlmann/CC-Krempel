...
local function receive_pongs()
    while not terminated do
        ...
        if packet.protocol == "route_expire" then
            print(packet.src_net .. "-" .. packet.src_id .. ": TTL expired")
        elseif packet.protocol == "route_notfound" then
            print(packet.src_net .. "-" .. packet.src_id .. ": Route not found")
        else
            for k, send_time in ipairs(pings) do
                if packet.payload == send_time then
                    table.insert(pongs, packet.timestamp - send_time)
                    print("#" .. k .. " " .. packet.src_net .. "-" .. packet.src_id .. " time=" .. string.format("%.1f", (packet.timestamp - send_time) / 3600.0) .. "t dis=" .. string.format("%.1f", packet.receive_distance) .. " sig=" .. string.format("%.1f", packet.signal_strength / 1024.0 * 100.0) .. "% sdr=" .. string.format("%.1f", (packet.receive_distance / (1024.0 - packet.signal_strength) * 100)) .. "%")
                end
            end
        end
    end
